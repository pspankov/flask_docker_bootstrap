...
from src.models import Customer, Store, Receipt, Product
...
class StoreSchema(graphene.ObjectType):
    ...
    def resolve_products(self, info):
        return [
            ...
            for product in Store().fetch(self._id).fetch_products()
        ]
...
class ReceiptSchema(graphene.ObjectType):
    ...
    def resolve_products(self, info):
        return [
            ProductSchema(**product)
            for product in Receipt().fetch(self._id).fetch_products()
        ]
...
class CustomerSchema(graphene.ObjectType):
    ...
    def resolve_receipts(self, info):
        return [
            ReceiptSchema(**receipt.as_dict()) for receipt in self.customer.receipts
        ]

    def resolve_products(self, info):
        return [
            ProductSchema(**product.as_dict()) for product in self.customer.products
        ]
