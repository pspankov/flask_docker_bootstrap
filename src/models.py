...
class Product(BaseModel):
    ...
    def as_dict(self):
        return {"name": self.name, "brand": self.brand, "category": self.category}
...
class Store(BaseModel):
    ...
    def fetch_by_name_and_address(self):
        return (
            Store.match(graph)
            .where(f'_.name = "{self.name}" AND _.address = "{self.address}"')
            .first()
        )

    def fetch_products(self):
        return [
            {**product[0].as_dict(), **product[1]}
...
class Customer(BaseModel):
    ...
    def __verify_receipt(self, receipt):
        ...
        existing_receipts = graph.run(
            f"MATCH ({customer_properties})-[relation:HAS]-({receipt_properties}) RETURN relation"
        ).data()
